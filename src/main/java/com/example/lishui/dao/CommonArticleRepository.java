...
public interface CommonArticleRepository extends JpaRepository<CommonArticle, Long> {
    ...
    Page<CommonArticle> findAll(Pageable pageable);

    @Operation(summary = "根据标题或发布人查找文章")
    @RestResource(path = "findAllByTitleOrAuthor")
    @Query(value = "select  u from #{#entityName} u where u.module=:module and u.tag=:tag and u.title like %:search% or u.username like %:search%")
    Page<CommonArticle> findAllByTitleOrUsername(@Param("module") String module,@Param("tag") String tag, @Param("search") String search, Pageable p);
...
//    @Query(value = "select  u from #{#entityName} u where u.module=:module and u.tag= :tag and u.createAt between :start and :end")
    Page<CommonArticle> findAllByModuleAndTagAndCreateAtBetween(@Param("module") String module,@Param("tag") String tag,@DateTimeFormat(pattern = "yyyy-MM-dd") @JsonFormat(timezone = "GMT+8", pattern = "yyyy-MM-dd") Date start,
    ...
    Page<CommonArticle> findAllByModuleAndTag(String module, String tag,Pageable p);
}

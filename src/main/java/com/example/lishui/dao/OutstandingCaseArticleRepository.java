...
public interface OutstandingCaseArticleRepository extends JpaRepository<OutstandingCaseArticle,Long> {
    ...
    List<OutstandingCaseArticle> findAllByTitleOrUsername(@Param("search") String search, Pageable p);
    ...
    List<OutstandingCaseArticle> findAllByCreateAtBetween(@DateTimeFormat(pattern = "yyyy-MM-dd") @JsonFormat(timezone = "GMT+8", pattern = "yyyy-MM-dd") Date start,
    ...
    List<OutstandingCaseArticle> findAllByTagEquals(String tag, Pageable p);
}
